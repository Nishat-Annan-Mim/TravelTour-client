...
export default function LoginPage() {
  ...
  const handleDemoLogin = async () => {
    setValue("email", "demo@trailnest.com");
    setValue("password", "demo12345");
    setServerError("");
    setLoading(true);
    try {
      await login("demo@trailnest.com", "demo12345");
      router.push("/");
    } catch {
      setServerError(
        "Demo account unavailable right now. Please try again shortly.",
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-md p-8">
        <div className="flex flex-col items-center mb-6">
          ...
          <p className="text-gray-500 text-sm mt-1">
            Log in to continue exploring tours
          </p>
        </div>
        ...
        <button
          ...
          className="w-full border border-primary text-primary font-medium rounded-lg py-2.5 mb-4 hover:bg-primary/5 transition"
        >
          Try Demo Login
        </button>
